...
# os.environ["OPENAI_API_KE"] = os.getenv('OPENAI_API_KEY')
...
def generate_answer(content: str, question: str) -> str:
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0,
                     api_key='as-D73mmid1JVABYjxT4_ncuw', base_url='https://gateway.agione.ai/openai/api/v2')
    prompt = PromptTemplate(
...
if __name__ == "__main__":
    ...
    query = "根据现有的网络与文档资料补充并生成一份培训文档大纲，内容对象为受训人员所以需要有详细的岗位知识和工作流程，需内容翔实有针对性直接面向新员工，并能有效提升员工的职业发展能力。"
    ...
    # Further query example
    query = f"根据这份培训大纲{answer}, 以及网络资料关于阿里巴巴，生成具体的培训文档内容，并进行审核，确保内容的准确性、完整性、及时性。"
    retrieved_documents = document_query_retriever.get_relevant_documents(query)
