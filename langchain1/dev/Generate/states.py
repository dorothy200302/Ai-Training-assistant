...
class AgentState(TypedDict):
    ...
    title: str             # 论文标题
    messages: str          # 系统消息历史
    area_of_paper: str     # 论文领域
    type_of_document: str  # 文档类型
    section_names: str     # 章节名称
    number_of_paragraphs: str  # 每个章节的段落数
    results: str           # 研究结果
    references: List[str]  # 参考文献列表
    
    # 主题句和写作指导相关
    review_topic_sentences: List[str]  # 主题句审查历史
    review_instructions: List[str]     # 审查指导历史

    task: str             # 当前任务
    plan: str             # 写作计划
    draft: str            # 当前草稿
    critique: str         # 评论意见
    cache: Set[str]       # 缓存的搜索结果
    content: List[str]    # 内容列表
    revision_number: int  # 修订版本号
    number_of_queries: int # 查询次数
    max_revisions: int    # 最大修订次数
    sentences_per_paragraph: int # 每段句子数
